...
def extract_cavity_features(cavities):
    """
    Extracts features from cavity images for KNN classification.
    ...
    """
    features = {}
    for direction, cavity in cavities.items():
        cavity_pixels = np.sum(cavity)  # Number of pixels in the cavity
        if cavity_pixels > 0:
            centroid = center_of_mass(cavity)  # Position of the centroid
        else:
            centroid = (0, 0)
        features[f'{direction}_pixels'] = cavity_pixels
        features[f'{direction}_centroid_x'] = centroid[0]
        features[f'{direction}_centroid_y'] = centroid[1]
    return features

def create_features_dataframe(isolated_digits, train_labels):
    """
    Creates a DataFrame of features extracted from isolated digit images.
    
    Parameters:
    - isolated_digits: List of isolated digit images as PIL images.
    
    Returns:
    - df: DataFrame containing features for each digit.
    """
    data = []
    for digit_index, digit_image in enumerate(isolated_digits):
        img = np.array(digit_image)
        cavities = find_cavities(img)
        features = extract_cavity_features(cavities)
        features['label'] = train_labels[digit_index]
...
def predict_digit(knn, scaler,image_path, train_labels):
    
    image = Image.open(image_path).convert('L')
    binary_image = binarized(image)
    isolated_digits = isolate_digits(binary_image,'predict_digit_')
    df = create_features_dataframe(isolated_digits, train_labels)
    X = df.drop(columns=['label'])
...
if __name__ == "__main__":
    ...
    df = create_features_dataframe(isolated_digits, labels)
    df = pd.concat([df] * 50, ignore_index=True)

    print(df)
    
    knn, scaler = train_knn_classifier(df)
    
    # Prediction example using a new image (provide path to the new image)
    prediction_image_path = "new_code_image.png"  # Replace with actual image path for prediction
    predictions = predict_digit(knn, scaler, prediction_image_path, labels)